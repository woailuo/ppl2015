...
$P$ under $\Theta$ and $\Gamma$, where \(\Theta\) is a mapping from function variables to function types and \(\Gamma\) is a type environment which includes variables.%%  We design the type system so that
...
\begin{myDef}[\(\sharp_{\rho}(\sigma)\), OK_{n}(P)]
  \( \sharp_{\malloc}(\sigma), \sharp_{\free}\) are functions to count the number of \(\Malloc\) and \( \Free \)actions in a sequence \( \sigma\) respectively. Function \(OK_n(P)\) is a function to guarantee that if a program executes sequence actions \( \sigma\) for any \(P'\) such that \(P\xLongrightarrow{\sigma}P'\), then the difference of \( \sharp_{\malloc}(\sigma)\) and \( \sharp_{\free}(\sigma)\) does not exceed the number \(n\).
\label{df:okn}
