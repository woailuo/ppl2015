...
so it makes an internal action and behaves like a substitution.

performs internal
action and then turns into \([\mu\alpha.P/\alpha]P\).
...
system. In the rule for assignment, it behaves like \(\bf 0\). In the
rule for \(\Free\), the deallocation command behaves like \(\Free\).
In the rule for \(\Malloc\), this let-expression behaves like
\(\Malloc;P\) in which \(\Malloc\) is the behavior type of
\(\Malloc()\) command and \(P\) is the behavior type of statement
\(s\). In the rule for other let-expressions like \rn{Tr-LetEq},
\rn{Tr-LetDref} and \rn{Tr-LetNull}, these expressions behave like
what the statement \(s\) behaves, for example, \(P\).
