...
action \(\rho\), \(P\) turns into \(P'\). The behavioral types
\(\Malloc\) and \(\Free\), intuitively, just perform \(\Malloc\) and
\(\Free\) actions respectively.  For the type \(P_1 + P_2\), it only
makes a choice between \(P_1\) and \(P_2\), so it performs the
...
\([\mu\alpha.P/\alpha]P\). For the behavioral type \(P_1;P_2\),
\(P_1\) make actions \(\rho\) until it turns into \(\bf 0\); the type
\(\bf{0}\)\(;P_2\) will perform internal action because the type \(bf
0\) represents do-nothing behavior, and then it turns into \(P_2\).
...
\vdash s : P$.  It expresses that the behavior of $s$ is abstracted by
a behavioral $P$ under the variable type environment \(\Gamma\) and
the function type environment \(\Theta\).
...
\(s\). In the rule for other let-expressions like \rn{Tr-LetEq},
\rn{Tr-LetDref} and \rn{Tr-LetNull}, they do not perform any
