...
The type $0$ abstracts the behavior of $SKIP$ and means "does nothing". $P_{1};P_{2}$ is for sequential exetution. $P_{1} + P_{2}$ is abstracted as contional. $\Malloc$ is the behavior of a program that allocates a memory cell exactly once. $\Free$ is for deallocating memory cell exactly once. $\mu \alpha. P$ is a recursive type.
