...
\maketitle
\begin{abstract}
We propose a type system to prevent programs infinitelly consuming memory cells. The main idea is based on the previours type system which augments pointer types with fractional ownerships and behavioral type system which abstract the behavior of programs. The simple idea in our work is to reject allocating a memory cell before recursive calls.
\end{abstract}
...
Safe memory deallocation so far mainly deals with memory leak for terminating programs. For example, the type system by Suenaga and Kobayashi \cite{DBLP:conf/aplas/SuenagaK09} guarrantees that (1) a well-typed program does not perform read/write/free operations to any deallocated memory cell and that (2) after execution of a well-typed program, all the memory cells are deallocated.
