...
(e.g., operating systems and Web servers)\footnote{Memory leaks in
  nonterminating programs is that a program consumes unbounded number
  of memory cells and goes overflow.} than terminating ones, the
...
execute \(h\) and \(h'\). This method is better than directly
modelchecking the original program, because for directly modelchecking
original one we should add assertions for guaranteeing it no overflow
and calculation for memory consumption after each allocation and
deallocation where they appear, whereas by the abstracted behavior we
only use two functions, each one consists of an assertion and a
calculation, to fulfill this task.

\todo{Why do we extract an abstract behavior and then pass it to
  a model checker, not directly modelchecking the original program?}
