...
operating systems and Web servers) than terminating ones, the analyses proposed so far put less emphasis
...
One may not see, in the example above, much difference between
...
former in many programs because a behavioral type focuses on actions
on allocations and deallocations, abstracting away the other features.
Although the current paper only focuses on the theoretical aspect, we
plan to conduct experiment with actual programs in future.
