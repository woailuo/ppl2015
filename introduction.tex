...
(e.g., operating systems and Web servers) than terminating
ones\footnote{Memory leaks in nonterminating programs is that a
  program consumes unbounded number of memory cells and goes
  overflow}, the analyses proposed so far put less emphasis to
nontermination; they rather verify \emph{partial} memory-leak freedom:
if a program terminates, then all the allocated memory cells are
deallocated.  We say a program is \emph{totally} memory-leak free if
