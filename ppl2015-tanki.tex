...
Function $PT_{v}(x) = (\emptyset,\emptyset)$ denotes that it receives a pointer variable $x$ and outputs a pair consisting of empty set only. $PT_{\Theta}(s) = (C, P)$ is a mapping from statements to a pair of constraints set $C$ and behavioral types $P$, where $\Theta$ is mapping from function names to function types. $PT(<D,s>) = (C, P)$ denotes that it receives a program and returns a pair $(C, P)$.
