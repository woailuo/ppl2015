...
\appendix
\section{Appendix}
\subsection{Proof for Lemma Preservation}
...
\begin{align}
  &OK_{n}(P) \label{eq2.1} \\
  &\Theta; \Gamma \vdash \FREE: P \label{eq2.2}
\end{align}

By the inversion lemma on \eqref{eq2.2}, we have:
\begin{align}
   &\Free \le P& \label{eq2.3}
\end{align}
From the definition of subtyping, the \eqref{eq2.3} and $\Free \xlongrightarrow{\Free} 0$, we get:\\
$\exists P''$ s.t.
\begin{align}
  &P \overset{\text{$\Free$}}{\Longrightarrow} P'', \ and & \label{eq2.4}\\
  &0 \le P''& \label{eq2.5}
\end{align}
We need to prove there exists $P'$ and $\Gamma'$ such that: \\
\begin{align}
  &\Theta; \Gamma' \vdash \SKIP: P',\  and & \label{eq2.4.5}\\
  &P \overset{\text{$\Free$}}{\Longrightarrow} P'& \label{eq2.4.6}
\end{align}
Take $P''$ as $P'$. \\
Then \eqref{eq2.4.6} holds. \\
