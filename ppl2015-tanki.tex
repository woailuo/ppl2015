...
Manual memory management primitives (e.g., \texttt{malloc} and \texttt{free} in C) often cause serious problems such as double frees, memory leaks, and read/write operations to a deallocated memory cell; we call such operations \emph{illegal accesses}. Verifying \emph{safe memory deallocation}\textendash a program not leading to such an unsafe state\textendash is an important problem.
...
The following BNF defines the language.
...
allocation and deallocation, and memory accesses with pointers. The
