...
The key idea of our approach is to decompose this problem into two subproblems: (1) partial correctness and (2) \emph{behavioral correctness}. The behavioral correctness means a program does not \emph{leak} memory. It is verified by behavioral type system which is mainly used to abstact the behavior of a program. Behavioral types are heavily used in the context of concurrent program verification \cite{DBLP:journals/lmcs/KobayashiSW06,DBLP:journals/tcs/IgarashiK04,DBLP:conf/esop/HondaVK98}. The main procedure to guarantee safe memory deallocation for non-terminating programs is that first checking the partial correctness by previous type system and then verifing memory-leak freedom by behavioral type system.
