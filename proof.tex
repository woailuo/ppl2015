...
According to the rule \rn{Tr-Skip}, we need to prove
...
From the rule \rn{Tr-Malloc}, we need to prove \(\sharp_{m}(P') -
...
From the rule \rn{Tr-Free}, we need to prove \(\sharp_{m}(P') - \sharp_{f}(P') \le n+1\).
...
\infax{ n' + \sharp_m(\rho) - \sharp_f(\rho) <  \sharp_{m}(\rho) + \sharp_{m}(\rho_1) - \sharp_{f}(\rho) -\sharp_{f}(\rho_1) \le n}
